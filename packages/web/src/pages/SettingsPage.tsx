...
function BlockchainSettings() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("Failed to save RPC URL:", e);
    } finally {
      setIsSaving(false);
    }
  };
          ...
          <div className="p-4 flex flex-col gap-3">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                Custom node URL for on-chain transactions (Devnet only)
              </p>
            </div>
            <div className="flex gap-2">
              <input
                ...
                placeholder={
                  loadingConfig ? "Loading..." : "https://api.devnet.solana.com"
                }
...
function GuardrailSettings() {
  ...
  const fetchConfig = useCallback(async () => {
    ...
    try {
      ...
      if (res.data) {
        ...
        try {
          parsedAllowlist = res.data.allowlist
            ? JSON.parse(res.data.allowlist)
            : [];
        } catch {
          parsedAllowlist = [];
        }
        ...
        const fetched = {
          ...
          confirmation_threshold: parseFloat(
            res.data.confirmation_threshold || "50",
          ),
  ...
  const handleRemoveAddress = (addr: string) => {
    setConfig({
      ...config,
      allowlist: config.allowlist.filter((a: string) => a !== addr),
    });
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 flex items-center justify-between bg-red-500/5 border border-red-500/20 rounded-xl shadow-sm">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground mr-4">
                Immediately halt all transaction signing across all agents.
              </p>
            </div>
            <button
              onClick={() =>
                setConfig({ ...config, kill_switch: !config.kill_switch })
              }
            ...
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${config.kill_switch ? "translate-x-6" : "translate-x-1"}`}
              />
            </button>
          </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <input
                ...
                onChange={(e) =>
                  setConfig({
                    ...config,
                    per_trade_limit: parseFloat(e.target.value),
                  })
                }
          ...
          <div className="p-4 flex items-center justify-between bg-card border border-border rounded-xl shadow-sm">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground mr-4">
                Total SOL volume allowed per 24h window.
              </p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <input
                ...
                onChange={(e) =>
                  setConfig({
                    ...config,
                    daily_volume_cap: parseFloat(e.target.value),
                  })
                }
                className="w-20 px-2 py-1 bg-secondary border border-border rounded text-sm font-mono text-right"
              />
          ...
          <div className="p-4 flex items-center justify-between bg-card border border-border rounded-xl shadow-sm">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground mr-4">
                Maximum slippage % allowed for DEX swaps.
              </p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <input
                ...
                onChange={(e) =>
                  setConfig({
                    ...config,
                    slippage_cap: parseFloat(e.target.value),
                  })
                }
                className="w-20 px-2 py-1 bg-secondary border border-border rounded text-sm font-mono text-right"
              />
            ...
            <div className="flex items-center gap-2 shrink-0">
              <input
                ...
                onChange={(e) =>
                  setConfig({
                    ...config,
                    cooldown_period: parseInt(e.target.value, 10),
                  })
                }
                className="w-20 px-2 py-1 bg-secondary border border-border rounded text-sm font-mono text-right"
              />
          ...
          <div className="p-4 flex items-center justify-between bg-card border border-border rounded-xl shadow-sm">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground mr-4">
                Trades above this SOL value require manual signing.
              </p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <input
                ...
                onChange={(e) =>
                  setConfig({
                    ...config,
                    confirmation_threshold: parseFloat(e.target.value),
                  })
                }
                className="w-20 px-2 py-1 bg-secondary border border-border rounded text-sm font-mono text-right"
              />
          ...
          <div className="p-4 space-y-3 bg-card border border-border rounded-xl shadow-sm md:col-span-2">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">
                Only allow transfers to these addresses. Leave empty to allow
                all.
              </p>
            </div>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 bg-secondary/50 border border-input rounded-md focus:outline-none focus:ring-1 focus:ring-primary font-mono text-xs"
            ...
            {config.allowlist.length > 0 ? (
              <div className="space-y-1.5 mt-2">
                {config.allowlist.map((addr: string) => (
                  <div
                    key={addr}
                    className="flex items-center justify-between px-3 py-2 bg-secondary/30 border border-border rounded-md group"
                  >
              ...
              <p className="text-xs text-muted-foreground italic">
                No addresses — all recipients allowed.
              </p>
            )}
          </div>
        </div>
...
export function SettingsPage() {
  ...
  const handleFetchModels = async () => {
    ...
    try {
      const client = new ApiClient(token);
      const providerName =
        newProvider === "custom" ? customName || "custom" : newProvider;
      ...
      setProviderError(
        e.message || "Failed to fetch models. Check API key and provider.",
      );
    } finally {
    ...
    <div className="flex flex-col h-full space-y-6 overflow-y-auto pr-4 pb-12">
      <header className="flex flex-col space-y-2 shrink-0">
        ...
        <p className="text-muted-foreground">
          Manage system, appearance, blockchain, and AI provider configurations.
        </p>
      </header>
      ...
      <div className="flex-1 space-y-6">
              ...
              <div className="bg-card border border-border rounded-xl divide-y divide-border shadow-sm">
                ...
                <div className="p-4 flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Connection to Core API
                    </p>
                  </div>
                  <code className="text-xs font-mono bg-secondary px-2 py-1 rounded-md text-primary font-medium">
                    {window.location.hostname}:7445
                  </code>
                </div>
              </div>
            </section>
              ...
              <div className="bg-card border border-red-500/20 rounded-xl divide-y divide-border shadow-sm">
                <div className="p-4 flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Wipe local session tokens & cache
                    </p>
                  </div>
              ...
              <div className="bg-card border border-border rounded-xl divide-y divide-border shadow-sm">
                <div className="p-4 flex flex-col gap-4">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Customize your dashboard aesthetic
                    </p>
                  </div>
        ...
        {/* Guardrails Tab */}
        {activeTab === "guardrails" && <GuardrailSettings />}
              ...
              <div className="bg-card border border-border rounded-xl divide-y divide-border shadow-sm">
                <div className="p-4 flex flex-col gap-3">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Select the primary model for general reasoning
                    </p>
                  </div>
                        ...
                        >
                          <div className="flex flex-col">
                            <span className="font-medium text-sm">
                              {p.name}{" "}
                              {p.is_primary === 1 && (
                                <span className="ml-2 text-[10px] bg-primary/20 text-primary px-1.5 py-0.5 rounded uppercase tracking-wider font-bold">
                                  Primary
                                </span>
                              )}
                            </span>
          ...
          <div className="w-full max-w-2xl bg-card border border-border rounded-xl shadow-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="px-6 py-4 border-t border-border flex justify-end gap-3 bg-secondary/10 shrink-0">
              ...
              <button
                ...
                disabled={
                  !selectedModel || (newProvider === "custom" && !customName)
                }
