...
export function registerOnboardCommand(program: Command) {
  program
    .command("onboard")
    .description(
      "Guided onboarding wizard — set up your first agent and provider",
    )
    .action(async () => {
      ...
      while (!providerReady) {
        ...
        if (clack.isCancel(providerName)) {
        ...
        if (isCustom) {
          ...
          const customName = await clack.text({
            ...
            validate: (val) =>
              val.length < 1 ? "Name cannot be empty" : undefined,
          });
          ...
          const inputBaseUrl = await clack.text({
            ...
            validate: (val) =>
              val.length < 1 ? "Base URL cannot be empty" : undefined,
          });
          if (clack.isCancel(inputBaseUrl)) {
            clack.cancel("Setup cancelled.");
            process.exit(0);
          }
            ...
            options: [
              {
                ...
                hint: "Most providers (Ollama, LM Studio, vLLM, etc.)",
              },
              { value: "anthropic", label: "Anthropic-compatible" },
            ],
          });
          if (clack.isCancel(compatChoice)) {
            clack.cancel("Setup cancelled.");
            process.exit(0);
          }
          ...
          model = await clack.text({
            ...
            validate: (val) =>
              val.length < 1 ? "Model ID cannot be empty" : undefined,
          });
          ...
          s2.stop(
            models ? `Found ${models.length} models` : "Failed to fetch models",
          );
          ...
          if (models && models.length > 0) {
            ...
            if (String(model) === "__manual__") {
              model = await clack.text({
                ...
                validate: (val) =>
                  val.length < 1 ? "Model name cannot be empty" : undefined,
              });
            }
          }
      ...
      if (clack.isCancel(createFirst) || !createFirst) {
        clack.log.warn(
          "⚠️ WARNING: Without an agent, Sigil cannot execute transactions or interact with the blockchain.",
        );
        clack.log.info(
          "You must initialize the main agent later by running `sigil agent init` before using Sigil.",
        );
      } else {
      ...
      try {
        ...
        if (!token) {
          s.stop(
            "Daemon started, but session token not found. Please run `sigil start` manually.",
          );
          clack.outro(
            "Setup finished. Run `sigil dashboard` or `sigil tui` once the daemon is ready.",
          );
          process.exit(0);
        }
        ...
        const uiChoice = await clack.select({
          ...
          options: [
            {
              value: "web",
              label: "Web UI (Recommended - Modern & Visual)",
              hint: "Opens in your default browser",
            },
            {
              value: "tui",
              label: "TUI (Terminal Interface)",
              hint: "Launches in this terminal",
            },
          ],
        });
        ...
        if (clack.isCancel(uiChoice)) {
          clack.outro(
            "Background process is running. Run `sigil dashboard` or `sigil tui` anytime.",
          );
          process.exit(0);
        }
        ...
        if (uiChoice === "web") {
          ...
          clack.outro(
            "Sigil Wallet is still running in the background. See you soon!",
          );
        }
      } catch (err) {
