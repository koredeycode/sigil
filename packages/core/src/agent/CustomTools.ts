...
export function createCustomTools(
  ...
  const getBalanceTool = new DynamicStructuredTool({
    name: "get_balance",
    description:
      "Check SOL balance and all SPL token holdings for this agent's wallet. Call this once at the start of a task if you need to know what you own.",
    schema: z.object({}),
    func: async () => {
      try {
  ...
  const requestAirdropTool = new DynamicStructuredTool({
    ...
    description:
      "Request SOL from the devnet faucet. Maximum 2 SOL per request.",
    schema: z.object({
      amount: z
        .number()
        .min(0.1)
        .max(2)
        .describe("Amount of SOL to request (max 2)"),
    }),
    func: async ({ amount }) => {
      try {
        logger.debug(
          `[Tool:request_airdrop] Requesting ${amount} SOL for ${agentName}`,
        );
  ...
  const transferSolTool = new DynamicStructuredTool({
    ...
    func: async ({ to, amount }) => {
      try {
        logger.debug(
          `[Tool:transfer_sol] Transferring ${amount} SOL to ${to} for ${agentName}`,
        );
        const keypair = await getKeypair(agentName);
  ...
  const getPortfolioSnapshotTool = new DynamicStructuredTool({
    ...
    description:
      "Get a full breakdown of holdings with percentages of total portfolio.",
    schema: z.object({}),
    func: async () => {
      try {
  ...
  const stakeSolTool = new DynamicStructuredTool({
    name: "stake_sol",
    description:
      "Delegate SOL to a validator on devnet. Creates a stake account and delegates. Subject to guardrails.",
    schema: z.object({
      amount: z.number().positive().describe("Amount of SOL to stake"),
      validatorAddress: z
        .string()
        .describe("Validator vote account address (base58)"),
    }),
    func: async ({ amount, validatorAddress }) => {
      try {
        logger.debug(
          `[Tool:stake_sol] Staking ${amount} SOL to ${validatorAddress} for ${agentName}`,
        );
        const keypair = await getKeypair(agentName);
        const lamports = solToLamports(amount);
        const validatorPubkey = new PublicKey(validatorAddress);

        const { transaction, stakeKeypair } = buildStakeSol(
          keypair.publicKey,
          lamports,
          validatorPubkey,
        );
        ...
        if (result.status === "confirmed") {
          ...
          const accountLink = formatExplorerLink(
            "address",
            stakeKeypair.publicKey.toBase58(),
          );
  ...
  const deactivateStakeTool = new DynamicStructuredTool({
    ...
    description:
      "Deactivate a stake account so it can be withdrawn after the cool-down epoch.",
    schema: z.object({
      stakeAccountAddress: z
        .string()
        .describe("Stake account address to deactivate (base58)"),
    }),
    func: async ({ stakeAccountAddress }) => {
      try {
        logger.debug(
          `[Tool:deactivate_stake] Deactivating ${stakeAccountAddress} for ${agentName}`,
        );
        const keypair = await getKeypair(agentName);
  ...
  const createTokenTool = new DynamicStructuredTool({
    name: "create_token",
    description:
      "Create a new SPL token mint on devnet with optional metadata (name, symbol, image URI). You become the mint authority. Subject to guardrails.",
    schema: z.object({
      decimals: z
        .number()
        .int()
        ...
        .max(9)
        .default(9)
        .describe("Token decimals (0-9, default 9)"),
      name: z
        .string()
        .max(32)
        .optional()
        .describe('Token name (e.g., "Sigil Test Token")'),
      symbol: z
        .string()
        .max(10)
        .optional()
        .describe('Token symbol/ticker (e.g., "SGT")'),
      uri: z
        .string()
        .optional()
        .describe(
          "Metadata URI pointing to a JSON file with image/description (optional)",
        ),
    }),
    func: async ({ decimals, name, symbol, uri }) => {
      try {
        logger.debug(
          `[Tool:create_token] Creating token with ${decimals} decimals for ${agentName}`,
        );
        const keypair = await getKeypair(agentName);

        // Build metadata if name and symbol are provided
        const metadata = name && symbol ? { name, symbol, uri } : undefined;
        const { transaction, mintKeypair } = await buildCreateToken(
          keypair.publicKey,
          decimals,
          connection,
          metadata,
        ...
        if (result.status === "confirmed") {
          ...
          const txLink = formatExplorerLink("tx", result.signature!);
          const mintLink = formatExplorerLink(
            "address",
  ...
  const mintTokensTool = new DynamicStructuredTool({
    name: "mint_tokens",
    description:
      "Mint SPL tokens to this wallet. You must be the mint authority. Subject to guardrails.",
    schema: z.object({
      mintAddress: z.string().describe("Token mint address (base58)"),
      amount: z
        .number()
        .positive()
        .describe("Amount of tokens to mint (in UI units, e.g., 100.5)"),
    }),
    func: async ({ mintAddress, amount }) => {
      try {
        logger.debug(
          `[Tool:mint_tokens] Minting ${amount} of ${mintAddress} for ${agentName}`,
        );
        const keypair = await getKeypair(agentName);
        ...
        const tx = await buildMintTokens(
          keypair.publicKey,
          mint,
          rawAmount,
          connection,
  ...
  const burnTokensTool = new DynamicStructuredTool({
    ...
    schema: z.object({
      ...
      amount: z
        .number()
        .positive()
        .describe("Amount of tokens to burn (in UI units)"),
    }),
    func: async ({ mintAddress, amount }) => {
      try {
        logger.debug(
          `[Tool:burn_tokens] Burning ${amount} of ${mintAddress} for ${agentName}`,
        );
        const keypair = await getKeypair(agentName);
        const mint = new PublicKey(mintAddress);
  ...
  const transferTokenTool = new DynamicStructuredTool({
    name: "transfer_token",
    description:
      "Send SPL tokens to another address. Creates the recipient's token account if needed. Subject to guardrails.",
    schema: z.object({
      mintAddress: z.string().describe("Token mint address (base58)"),
      to: z.string().describe("Recipient Solana address (base58)"),
      amount: z
        .number()
        .positive()
        .describe("Amount of tokens to send (in UI units)"),
    }),
    func: async ({ mintAddress, to, amount }) => {
      try {
        logger.debug(
          `[Tool:transfer_token] Sending ${amount} of ${mintAddress} to ${to} for ${agentName}`,
        );
        const keypair = await getKeypair(agentName);
        const mint = new PublicKey(mintAddress);
        const recipient = new PublicKey(to);
        ...
        const tx = await buildTransferToken(
          keypair.publicKey,
          recipient,
          mint,
          rawAmount,
  ...
  const closeEmptyTokenAccountsTool = new DynamicStructuredTool({
    ...
    description:
      "Close all zero-balance SPL token accounts to reclaim rent SOL. Subject to guardrails.",
    schema: z.object({}),
    func: async () => {
      try {
        logger.debug(
          `[Tool:close_empty_token_accounts] Scanning for ${agentName}`,
        );
        const keypair = await getKeypair(agentName);
  ...
  const swapTokensTool = new DynamicStructuredTool({
    ...
    schema: z.object({
      ...
      amount: z
        .number()
        .positive()
        .describe("Amount of input token to swap (in UI units)"),
      slippageBps: z
        .number()
        .int()
        .min(1)
  ...
  const sendMemoTool = new DynamicStructuredTool({
    name: "send_memo",
    description:
      "Send an on-chain memo message. Useful for tagging transactions with human-readable notes. Subject to guardrails.",
    schema: z.object({
      message: z
        .string()
        .max(566)
        .describe("Memo message to write on-chain (max 566 bytes)"),
    }),
    func: async ({ message }) => {
      try {
  ...
  const fetchPriceTool = new DynamicStructuredTool({
    name: "fetch_price",
    description:
      "Get the current USD price of a token by its mint address or well-known symbol (SOL, USDC, etc).",
    schema: z.object({
      token: z
        .string()
        .describe("Token mint address (base58) or symbol like SOL, USDC, BONK"),
    }),
    func: async ({ token }) => {
      try {
  ...
  const getTpsTool = new DynamicStructuredTool({
    name: "get_tps",
    description:
      "Get the current transactions-per-second (TPS) on the Solana devnet cluster.",
    schema: z.object({}),
    func: async () => {
      try {
  ...
  const getSlotInfoTool = new DynamicStructuredTool({
    name: "get_slot_info",
    description:
      "Get current slot, epoch, and block height information from the Solana devnet cluster.",
    schema: z.object({}),
    func: async () => {
      try {
  ...
  const listValidatorsTool = new DynamicStructuredTool({
    ...
    description:
      "List active validators on the Solana devnet cluster with their vote accounts, stake, and commission. Useful for choosing a validator before staking.",
    schema: z.object({
      limit: z
        .number()
        .int()
        ...
        .max(20)
        .default(10)
        .describe("Number of top validators to return (default 10, max 20)"),
    }),
    func: async ({ limit }) => {
      try {
  ...
  const getStakePositionsTool = new DynamicStructuredTool({
    ...
    description:
      "View all stake accounts owned by this wallet, their status (active/deactivating/inactive), validator, and balance.",
    schema: z.object({}),
    func: async () => {
      try {
