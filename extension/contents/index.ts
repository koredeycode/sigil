...
console.log("[Sigil Extension] Isolated content script successfully running. Injecting provider...");

// Bridge messages from the webpage (MAIN world) to the background script (ISOLATED world)
window.addEventListener("message", (event) => {
  // Only accept messages from the same window
  if (event.source !== window || !event.data || event.data.target !== "sigil-content") {
    return;
  }
...
try {
  ...
  script.src = inpageUrl.startsWith("chrome-extension://") ? inpageUrl : chrome.runtime.getURL(inpageUrl);
